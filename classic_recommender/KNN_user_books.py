...
class Personal_KNN_recommender:
    def __init__(self, mode=0):
        self.index = pd.read_csv('../data/personal/books.csv')
        self.reader = Reader()
        self.ratings = pd.read_csv('../data/personal/book_ratings.csv')
        self.testings = pd.read_csv('../data/personal/book_test.csv')
...
result = test.recommend(712, 10)
for i in result:
     print(i)


recommend_res = test.recommend(id='508', k_nearest=8, n_items=10)
original_id_res = [recommender.algo.trainset.to_raw_iid(id) for id in recommend_res]
#for id in original_id_res:
 #   print(id2name[id])
recommend_result = recommender.recall_and_precision(test=['12','2485',5.0],k_nearest=8, n_items=10)
